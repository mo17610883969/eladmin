...
public class DeployServiceImpl implements DeployService {
	...
	public PageResult<DeployDto> queryAll(DeployQueryCriteria criteria, Pageable pageable) {
		return ServiceHelper.toPageResult(
                deployRepository.findAll((root, query, cb) -> QueryHelp.getPredicate(root, criteria, cb), pageable),
                deployMapper);
	}

	@Override
	public List<DeployDto> queryAll(DeployQueryCriteria criteria) {
		return deployMapper.toDto(deployRepository.findAll((root, query, cb) -> QueryHelp.getPredicate(root, criteria, cb)));
	}

	@Override
	public DeployDto findById(Long id) {
		return ServiceHelper.findById(deployRepository, deployMapper, id, ENTITY_NAME, Deploy::new);
	}
	...
	public void update(Deploy resources) {
		Deploy deploy = ServiceHelper.findByIdRaw(deployRepository, resources.getId(), ENTITY_NAME, Deploy::new);
	...
	public void delete(Set<Long> ids) {
		ids.forEach(deployRepository::deleteById);
	}
